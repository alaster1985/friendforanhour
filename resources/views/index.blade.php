                    ...
                    @foreach($profilesForLowerBlocks as $lProfile)
                        <div class="card" style="width: 18rem;">
                            <a class="profile-img-link" href="profile?prf={{$lProfile->id}}">
                                <img class="User_image card-img-top" src="{{asset($lProfile->profilePhoto()->where([['main_photo_marker', '=', 1], ['is_deleted', '=', 0]])->first()->photo_path ?? 'profilepictures/' .$lProfile->gender_id . '.jpg')}}">
                            </a>
                            <div class="card-body">
                                <h5 class="card-title">
                                ...
                                @endforeach
                                    ...
                                    <span class="name_serwise_cart name_serwise_cart_link bottom-card-section">
                                        @if(isset(Auth::user()->profile_id) && Auth::user()->profile_id != $lProfile->id)
                                            ...
                                            @csrf
                                                <button class="btn btn-primary btn-lg btn-block" type="submit">Написать</button>
                                            </form>
                                        @endif                                        
                                    </span>                                    
                                @guest
